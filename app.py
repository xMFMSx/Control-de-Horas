...
if not st.session_state.autenticado:
    ...
    if st.session_state.get("cambiando_password", False):
        ...
        # --- VISTA 2: RESUMEN MENSUAL CON EXPANDERS AL HACER CLIC EN CADA LÍNEA ---
            ...
            if registros_tabla:
                st.write("### 📋 Jornadas Registradas (Haz clic en una línea para editar)")
                ...
                for r in registros_tabla:
                    ...
                    # Formato de etiqueta para el expander estilo pantalla principal
                    etiqueta_linea = f"📅 Día {d:02d} ({nom_dia}) | {r['ENTRADA']} - {r['SALIDA']} | Obra: {r['OBRA']}"
                    if es_festivo:
                        etiqueta_linea += " [FERIADO]"

                    with st.expander(etiqueta_linea):
